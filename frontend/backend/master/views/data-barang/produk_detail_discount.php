...
");
	...
	$gvAttProdakItem=[
		...
		[
			...
			'mergeHeader'=>false,
			'noWrap'=>false,
			'filter'=>ArrayHelper::map(Product::find()->where(['ACCESS_GROUP'=>$user,'STATUS'=>1])->orderBy(['PRODUCT_ID'=>SORT_DESC,'STORE_ID'=>SORT_DESC])->all(),'PRODUCT_NM','PRODUCT_NM'),
			'filterType'=>GridView::FILTER_SELECT2,
			'filterWidgetOptions'=>['pluginOptions'=>['allowClear'=>true]],	
			'filterInputOptions'=>['placeholder'=>'-Pilih-'],
			'filterOptions'=>[],
			'value'=>function($data) {				
				return Html::tag('div', $data->PRODUCT_NM, ['data-toggle'=>'tooltip','data-placement'=>'left','title'=>'click to Product Items ','style'=>'cursor:default;']);				
			},	
			//gvContainHeader($align,$width,$bColor)
	...
	$gvAllStoreItem=GridView::widget([
		...
		'rowOptions'   => function ($model, $key, $index, $grid) {
			...
			$btnclick= ['ondblclick' => '
				$.pjax.reload({
					url: "'.Url::to(["/master/data-barang/index-discount?productid=".$model->PRODUCT_ID.""]).'",
