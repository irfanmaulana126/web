		...
		switch ($akunkode) {
			...
			case '3-1002':
					foreach($dinamikField[0] as $rows => $val){
						...
						if($splt[0]=='IN'){
							$nmField1[]=$rows;		//FULL FIELD NAME
							$nmLabel[]=$splt[0];	//SPLIT LABEL NAMEPRODUK_JUALPPNDISCOUNT
							$aryFieldColomn[]=['ID' =>$inc, 'ATTR' =>['FIELD'=>$rows,'SIZE'=>'7px','label'=>'JUAL PPN DISCOUNT','align'=>'right','group'=>false,'pageSummary'=>false,'BCOLOR'=>'#d9edf7','mergeHeader'=>true]];
							$inc=$inc+1;
						}	
					};	
				break;	
